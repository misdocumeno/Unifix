    ...
    if (argc >= 1) {
        ...
        if (currentState != STATE_NO_MIX && newTeam != L4D2Team_Spectator && IsHuman(client)) {
            ...
            if (!hSwapWhitelist.GetValue(authId, allowedTeam) || allowedTeam != newTeam) {
                CPrintToChat(client, "{blue}[{default}Mix Manager{blue}]{default} You can not join a team without being picked.");
                return Plugin_Stop;
            }
        }
...
public Action Cmd_MixStop(int client, int args) {
    if (currentState != STATE_NO_MIX) {
        StopMix();
        CPrintToChatAll("{blue}[{default}Mix Manager{blue}]{default} Stopped by admin \x03%N\x01.", client);
    } else {
        CPrintToChat(client, "{blue}[{default}Mix Manager{blue}]{default} Not currently started.");
    }
}
...
{
    if (currentState != STATE_NO_MIX) {
        CPrintToChat(client, "{blue}[{default}Mix Manager{blue}]{default} Already started.");
        return Plugin_Handled;
    } else if (!isMixAllowed) {
        CPrintToChat(client, "{blue}[{default}Mix Manager{blue}]{default} Not allowed on live round.");
        return Plugin_Handled;
    }
    ...
    if (mixConditions == COND_START_MIX || mixConditions == COND_START_MIX_ADMIN) {
        if (mixConditions == COND_START_MIX_ADMIN) {
            CPrintToChatAll("{blue}[{default}Mix Manager{blue}]{default} Started by admin \x03%N\x01.", client);
        } else {
            CPrintToChatAll("{blue}[{default}Mix Manager{blue}]{default} \x03%N {default}has voted to start a Mix.", client);
            CPrintToChatAll("{blue}[{default}Mix Manager{blue}]{default} Started by vote.");
        }
        ...
        CPrintToChatAll("{blue}[{default}Mix Manager{blue}]{default}  \x03%N {default}has voted to start a Mix. (\x05%d \x01more to start)", client, MIN_MIX_START_COUNT - mixCallsCount);

    } else if (mixConditions == COND_HAS_ALREADY_VOTED) {
        CPrintToChat(client, "{blue}[{default}Mix Manager{blue}]{default} You already voted to start a Mix.");
    ...
    if (action == MenuAction_Select) {
        if (currentState == STATE_FIRST_CAPT || currentState == STATE_SECOND_CAPT) {
            ...
            if (team == L4D2Team_Spectator || (team == L4D2Team_Infected && survivorsPick == 1) || (team == L4D2Team_Survivor && survivorsPick == 0)) {
                CPrintToChatAll("{blue}[{default}Mix Manager{blue}]{default} Captain \x03%N {default}found in the wrong team, aborting...", param1);
                StopMix();
                ...
                if (SwapPlayerToTeam(authId, team, 0)) {
                    ...
                    if (pickCount == 4) {
                        ...
                        CPrintToChatAll("{blue}[{default}Mix Manager{blue}]{default} Teams are picked.");
                        StopMix();
                    } else {
                        survivorsPick = survivorsPick == 1 ? 0 : 1;
                    } 
                } else {
                    CPrintToChatAll("{blue}[{default}Mix Manager{blue}]{default} The team member who was picked was not found, aborting...", param1);
                    StopMix();
                }
            }
        }
    ...
    switch(currentState) {
        case STATE_FIRST_CAPT: {
            ...
            if (SwapPlayerToTeam(currentMaxVotedCaptAuthId, L4D2Team_Survivor, numVotes)) {
                ...
                CPrintToChatAll("{blue}[{default}Mix Manager{blue}]{default} Failed to find first captain with at least 1 vote from spectators, aborting...");
                StopMix();
            }
        ...
        case STATE_SECOND_CAPT: {
            ...
            if (SwapPlayerToTeam(currentMaxVotedCaptAuthId, L4D2Team_Infected, numVotes)) {
                ...
                CPrintToChatAll("{blue}[{default}Mix Manager{blue}]{default} Failed to find second captain with at least 1 vote from spectators, aborting...");
                StopMix();
            }
        ...
        if (Menu_Initialise()) {
            ...
            if (captain > 0) {
                if (GetSpectatorsCount() > 0) {
                    ...
                    CPrintToChatAll("{blue}[{default}Mix Manager{blue}]{default} No more spectators to choose from, aborting...");
                    StopMix();
                    return Plugin_Stop;
                }
            } else {
                CPrintToChatAll("{blue}[{default}Mix Manager{blue}]{default} Failed to find the captain, aborting...");
                StopMix();
                return Plugin_Stop;
            }
...
public bool SwapPlayerToTeam(const char[] authId, L4D2Team:team, numVotes)
{
    ...
    if (foundClient) {
        ...
        switch(currentState) {
            case STATE_FIRST_CAPT: {
                CPrintToChatAll("{blue}[{default}Mix Manager{blue}]{default} First captain is \x03%N\x01. (\x05%d \x01votes)", client, numVotes);
            }
            
            case STATE_SECOND_CAPT: {
                CPrintToChatAll("{blue}[{default}Mix Manager{blue}]{default} Second captain is \x03%N\x01. (\x05%d \x01votes)", client, numVotes);
            }

            case STATE_PICK_TEAMS: {
                if (survivorsPick == 1) {
                    CPrintToChatAll("{blue}[{default}Mix Manager{blue}]{default} \x03%N \x01was picked (survivors).", client)
                } else {
                    CPrintToChatAll("{blue}[{default}Mix Manager{blue}]{default} \x03%N \x01was picked (infected).", client)
                }
            }
        }
    }
...
public void OnClientDisconnect(client)
{
    ...
    {
        CPrintToChatAll("{blue}[{default}Mix Manager{blue}]{default} Captain \x03%N \x01has left the game, aborting...", client);
        StopMix();
    }
}
